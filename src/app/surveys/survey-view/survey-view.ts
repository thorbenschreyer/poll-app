...
export class SurveyView {
  ...
  getAnswerPercentage(questionId: string | undefined, answerId: string | undefined): number {
    ...
    const databaseResponses = this.questionResponses()[questionId] ?? 0;
    const hasLocalSelection = this.getSelectedAnswers(questionId).length > 0;
    const responses = databaseResponses + (hasLocalSelection ? 1 : 0);
    if (responses === 0) {
      return 0;
    }
   ...
   */
  getPreviewAnswerVotes(questionId: string | undefined, answerId: string | undefined): number {
    if (!questionId || !answerId) {
      return 0;
    }
    const databaseVotes = this.getAnswerVotes(answerId);
    const isLocallySelected = this.getSelectedAnswers(questionId).includes(answerId);
